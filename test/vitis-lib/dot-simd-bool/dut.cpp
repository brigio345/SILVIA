...
void __dut(hls::stream<WideType<__din_t, PAR>::t_TypeInt> &din0Stream,
           hls::stream<WideType<__din_t, PAR>::t_TypeInt> &din1Stream,
           hls::stream<WideType<din_t, PAR>::t_TypeInt> &din2Stream,
           hls::stream<dout_t> &dout0Stream, hls::stream<dout_t> &dout1Stream) {
  dout_t dout0 = 0, dout1 = 0;
  for (auto i = 0; i < N / PAR; i++) {
#pragma HLS pipeline

    WideType<__din_t, PAR> din0 = din0Stream.read();
    WideType<__din_t, PAR> din1 = din1Stream.read();
    WideType<din_t, PAR> din2 = din2Stream.read();
...
void dut(__din_t *din0, __din_t *din1, din_t *din2, dout_t &dout0,
         dout_t &dout1) {
...
#pragma HLS array_partition variable = din2 type = cyclic factor = PAR dim = 0
  ...
  hls::stream<WideType<__din_t, PAR>::t_TypeInt> din0Stream("din0Stream");
  hls::stream<WideType<__din_t, PAR>::t_TypeInt> din1Stream("din1Stream");
  hls::stream<WideType<din_t, PAR>::t_TypeInt> din2Stream("din2Stream");
  hls::stream<dout_t> dout0Stream("dout0Stream");
  hls::stream<dout_t> dout1Stream("dout1Stream");
#pragma HLS dataflow
  readVec2Stream<__din_t, PAR>(din0, N, din0Stream);
  readVec2Stream<__din_t, PAR>(din1, N, din1Stream);
  readVec2Stream<din_t, PAR>(din2, N, din2Stream);
