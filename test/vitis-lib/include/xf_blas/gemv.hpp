...
void gemv(const unsigned int p_m,
          ...
          hls::stream<typename WideType<t_MacDataType, 1>::t_TypeInt>& p_y) {
 ...
 */
template <typename t_DataType, unsigned int t_LogParEntries, typename t_MacDataType = t_DataType, typename t_IndexType = unsigned int>
void gemv(const unsigned int p_m,
          const unsigned int p_n,
          ...
          hls::stream<typename WideType<t_MacDataType, 1>::t_TypeInt>& p_y,
          hls::stream<typename WideType<t_MacDataType, 1>::t_TypeInt>& p_yr) {
#ifndef __SYNTHESIS__
    assert(p_n % (1 << t_LogParEntries) == 0);
#endif
    const unsigned int l_numIter = p_n >> t_LogParEntries;
    hls::stream<typename WideType<t_MacDataType, 1>::t_TypeInt> l_x, l_y;
#pragma HLS DATAFLOW
    gemv<t_DataType, t_LogParEntries, t_IndexType>(p_m, p_n, p_M, p_x, l_x);
    scal<t_DataType, 1, t_MacDataType, t_IndexType>(p_m, p_beta, p_y, l_y);
